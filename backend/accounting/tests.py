...
class AccountingFoundationTests(TestCase):
    ...
    def test_unbalanced_entry_is_rejected_without_entry(self):
        with self.assertRaises(JournalValidationError):
            post_journal(number="JE-1405-00002", posting_date="2026-09-06", description="Invalid", lines=[{"account": self.cash, "debit": 100}, {"account": self.revenue, "credit": 99}])
        from .models import JournalEntry
        self.assertFalse(JournalEntry.objects.filter(number="JE-1405-00002").exists())

    def test_duplicate_number_is_rejected(self):
        post_journal(number="JE-1405-00003", posting_date="2026-09-06", description="First", lines=[{"account": self.cash, "debit": 10}, {"account": self.revenue, "credit": 10}])
        with self.assertRaises(IntegrityError):
            post_journal(number="JE-1405-00003", posting_date="2026-09-06", description="Duplicate", lines=[{"account": self.cash, "debit": 10}, {"account": self.revenue, "credit": 10}])
    ...
    def test_parent_account_cannot_receive_posting(self):
        ...
        with self.assertRaises(JournalValidationError):
            post_journal(number="JE-1405-00004", posting_date="2026-09-06", description="Invalid parent", lines=[{"account": parent, "debit": 10}, {"account": self.revenue, "credit": 10}])
